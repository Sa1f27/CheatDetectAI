...
# Load YOLOv5m model (download yolov5m.pt first)
model = torch.hub.load('ultralytics/yolov5', 'custom', path='yolo-models\yolov5l.pt')
        ...
        if frame_count % N == 0:
            ...
            for _, row in detections.iterrows():
                if row['confidence'] > 0.3:
                    label = row['name']
                    if label in ["cell phone", "book", "laptop"]:  # Only log cheating-related objects
