...
try:
    import win32gui  # For screen monitoring (Windows)
except ImportError:
    win32gui = None
...
def log_event(message):
    """Thread-safe logging of suspicious events with a timestamp."""
    with log_lock:
        timestamp = time.time()
...
# Load YOLOv8 Model
# -------------------------------
# Ensure you have the .pt file (e.g., yolov8l.pt) downloaded in the specified folder.
yolo_model = YOLO("yolo-models/yolov8l.pt")  # For best accuracy on GPU; you can use yolov8m.pt for balance.
...
def estimate_head_pose(landmarks, image_size):
    ...
    """
    # Selected landmark indices: nose tip (1), chin (152), left eye corner (33), right eye corner (263), left mouth corner (61), right mouth corner (291)
...
def detect_hands(frame):
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            # Calculate normalized bounding box for hand
...
def screen_monitoring():
    ...
    This function works only on Windows (requires win32gui).
    """
    allowed_windows = ["Zoom", "Teams", "InterviewApp"]  # Adjust these as needed
    while True:
        if win32gui:
            active_window = win32gui.GetWindowText(win32gui.GetForegroundWindow())
            if active_window and not any(allowed in active_window for allowed in allowed_windows):
                log_event(f"Suspicious active window detected: {active_window}")
        time.sleep(2)
...
def analyze_video(video_path):
    """
    Processes the video frame-by-frame, performing face, hand, and object detection.
    Also estimates head pose and gaze direction.
    Logs any suspicious behavior.
    """
    ...
    while cap.isOpened():
        ...
        if face_results.multi_face_landmarks:
            ...
            for face_landmarks in face_results.multi_face_landmarks:
                ...
                if abs(pitch) > 20 or abs(yaw) > 20:
                    log_event(f"Unusual head pose: pitch={pitch:.2f}, yaw={yaw:.2f}, roll={roll:.2f}")
        ...
        for result in detections.boxes:
            ...
            if confidence > 0.3 and label in ["cell phone", "book", "laptop", "earphone", "screen"]:
                log_event(f"{label.capitalize()} detected (confidence: {confidence:.2f})")
        
        # Hand Detection
        hands = detect_hands(frame)
        if hands:
            log_event("Hand(s) detected")
        
        # (Optional) Visualization: Uncomment to see live processing
...
def detect_cheating(logs):
    """
    Sends the collected logs to an LLM (via Groq) for a final detailed analysis.
    """
    if not logs:
    ...
    prompt = (
        ...
        "Focus on unusual gaze direction, abnormal head pose, unauthorized objects (e.g., cell phone, book, laptop, earphone, screen), "
...
if __name__ == "__main__":
    video_path = r"videos/cheat-2.mp4"  # Path to your interview video

    # Start background audio monitoring
    stop_audio = start_audio_monitoring()
    
    # Start screen monitoring thread (if win32gui is available)
    if win32gui:
        screen_thread = threading.Thread(target=screen_monitoring, daemon=True)
        screen_thread.start()
    
    print("Analyzing video for cheating detection...")
    ...
    # Stop audio monitoring (non-blocking stop)
